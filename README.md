...
- **AI review default-off, gated-target only.** Cloudflare Workers AI review is wired into the pipeline, but it runs only on the gated-target surface (PyPI/npm workflow-gate mode) and is additionally gated by the per-organization Flagship `ai-review` flag, defaulting to unavailable. The staged-publish hot path is deterministic-only and never runs AI — no added cost or latency on the publish. Deterministic findings are the review authority unless a complete, schema-valid AI review is enabled; AI remains advisory and cannot downgrade deterministic findings.
...
- AI triage (Workers AI JSON-mode review of targeted changed-file evidence) is wired but runs only on the gated-target surface (workflow-gate mode) and is default-off behind the per-organization Flagship `ai-review` flag. The staged-publish path never runs it. When disabled or on the staged-publish path, scans record AI review as unavailable and rely on deterministic findings.
