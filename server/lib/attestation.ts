...
/**
 * RFC 8037 names this key's JOSE algorithm `EdDSA`; Node's WebCrypto exports it
 * as `Ed25519` (the newer draft spelling). Only the first imports in workerd —
 * the other is rejected with a `DataError` — so a key generated by the very
 * `node -e` snippet the docs hand operators would load as "no key configured"
 * and degrade every attestation endpoint to `503`. The curve is already pinned
 * by `crv` above; `alg` is a label, and both labels name this curve.
 */
function normalizeEd25519Alg(jwk: JsonWebKey): JsonWebKey {
  if (jwk.alg !== "Ed25519") return jwk;
