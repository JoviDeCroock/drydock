...
- **Section labels:** A small mono uppercase label with a trailing rule, rendered as a real `h2` (`h3`/`p` escape hatches) so report sections keep a navigable heading outline under the page `h1`; an `aside` slot places trailing metadata (e.g. a count) after the rule. Replaces the generic visual H1/H2 stack with something more document-shaped. For a bare 11px mono label without the rule, use the `MonoLabel` primitive instead of copy-pasting the class string — page-local copies drift. The trailing rule **is** the section divider: never add a `border-t`, `border-b`, or `<hr>` on the same boundary as a `SectionLabel` (or a summary/header that contains one) — it stacks a second hairline and reads as a double border. This has regressed repeatedly; when a section needs more separation, use spacing, not another rule.
...
- **Selected file:** `bg-surface-2` row with `text-ink` and `aria-pressed` (the landing preview and the workbench share this contract). Border-radius `4px` on the row so the selection sits inside the tree padding.
