...
| 2026-07-19 | Added/bumped dependencies link to that dependency's own public diff; the public diff page gains a Manifest changes section.                                              | The new `dependency.added` / `dependency.major-bump` findings tell reviewers to inspect the dependency's own releases, but the manifest diff offered no path there. Each added/bumped row now carries a right-aligned mono 11px underlined `view diff` text link (no glyph — `↗` is not in the allowed set) opening in a new tab so the parent review stays put. Bumps with concrete floor versions link the version pair directly; added deps use a new package-only `/diff/<name>` route that resolves the latest published pair. `PackageJsonDiffView` moved to `src/components/` and now renders on the public diff page too (PyPI scans suppress the links — their dependencies are not npm packages).                                                                                                                                                                                                                                                                                                                                                                          |
