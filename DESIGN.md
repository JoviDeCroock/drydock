...
Every page is a min-height flex column: header → content (`main`, grows) → site footer pinned to the bottom. The footer matches the page's content width and carries the brand mark, a mono `11px / --fg-subtle` line (`Pre-publish review for npm, PyPI, and VS Code · © Drydock`), and `Docs` / `Feedback` / `Security` links — the only trust/legal surface, so it stays on every route (marketing _and_ app). `width="narrow"` pages (auth, 404, invite) are single short cards: they vertically center in the space between header and footer (via `my-auto`, which never clips over-tall content) rather than pinning to the top.
...
| 2026-07-14 | Landing rebuilt as an incident-led argument: a documented supply-chain incident ledger, the Hold → Review → Decide checkpoint, a scan-on-view product proof, coverage listed by real rule IDs, and an explicit free-for-open-source statement. | The page toured features before establishing why artifact review exists. The new order argues from history: seven documented incidents (2018–2025, npm + PyPI + the xz tarball) where the attack was visible only in the published artifact, each closed by an accent-callout claim (the callout rule's "key trust boundary" use). The proof card performs its scan once when first scrolled into view — pulsing mono loading line, then staggered opacity fade-ins (composed from the codified motion patterns; prerender, no-JS, and reduced-motion render the finished state). Deterministic coverage is a document-shaped `dl` of real rule IDs tied back to ledger incidents; integration is two equal contract cards with `best for`/`held by`/`decision` fields matching the docs; sponsorship sits beside the free-for-OSS commitment it funds. No new tokens, widths, glyphs, or chart shapes.                                |
