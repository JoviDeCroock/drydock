...
- **Hero product proof:** On wide screens, hero copy may share the `1160px` app shell with one real product-cutaway Card. The copy remains capped by the canonical hero widths; the proof uses existing report/diff primitives and stacks below the copy on smaller screens.
...
- No stock-photo or illustrated hero. The hero is type + brand mark + a real product Card.
...
| 2026-07-14 | Landing becomes a product-led release-gate narrative with wide-screen product proof and a signal-driven guided review.                                                | The accurate but documentation-shaped page repeated mechanics before making the artifact-review gap tangible. The new order is outcome + real product proof → release gap → guided evidence → deterministic coverage → integration modes → trust contract → operations → closing action. A compact report cutaway may sit beside hero copy inside the existing 1160px shell; it stacks on smaller screens and uses the same diff, severity, and recommendation language as the application. Sponsorship moves next to the trust story instead of interrupting the hero-to-product flow.                                                                                                                                                                                                                                                                                                                                                |
