...
// Server shape from `getReleaseAuthorityForGate`. The snapshot and delta are
// already validated by the tolerant readers server-side, so the UI can rely on
// their types — but both stay nullable, because a row can hold a blob that
// failed validation.
...
export interface PersistedScanDetail {
  ...
  riskSummary?: ScanRiskSummary | null;
  // Gate-level release-authority record, shared by every package scan of a
  // monorepo gate. Null for staged-publish scans and for gates reviewed before
  // authority capture existed — which reads as "not assessed", not "unchanged".
...
export const ScanDetailModel = createModel((id: string) => {
  ...
  // Release-authority delta for the gate, plus whether org policy holds the
  // release until an authority change is explicitly accepted in the dialog.
