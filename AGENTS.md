...
- `server/` — Hono Worker. `index.ts` mounts routes under `/api/*`. The worker is the deploy target (`main` in `wrangler.jsonc`).
  ...
  - `lib/ai-review.ts` — Workers AI JSON-mode reviewer. Wired into `scan-pipeline.ts` via `maybeRunAiReview`, but it **only runs on the gated-target surface** (`runScanPipeline`'s `surface: "gated-target"`, i.e. the workflow-gate path) and is additionally **gated by the per-organization `ai-review` Flagship flag, off by default** (planned paid-tier feature). The staged-publish hot path passes `surface: "staged-publish"` and is deterministic-only — `maybeRunAiReview` short-circuits before consulting Flagship, so no AI cost/latency. On the gated-target surface it runs only when Flagship returns `true` for the scanning org; otherwise the pipeline records an `unavailable` AI review. Don't change the surface gating or the default-off Flagship gating without a feature decision.
...
- Trust boundary: package bytes are untrusted evidence. Deterministic findings are authoritative. The AI reviewer is wired in but runs only on the gated-target surface (workflow-gate path) and is gated off-by-default behind the per-organization `ai-review` Flagship flag (planned paid-tier feature); the staged-publish hot path is deterministic-only and never runs it. When enabled it only sees changed files, treats package contents as hostile evidence, and cannot downgrade deterministic findings.
