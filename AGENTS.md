...
- `server/` — Hono Worker. `index.ts` mounts routes under `/api/*`. The Worker is the deploy target (`main` in `wrangler.jsonc`).
  ...
  - `lib/release-authority/` — bounded workflow-YAML reading, the release-authority snapshot/delta, and gate capture; see `docs/release-authority.md`.
...
- Package bytes are hostile evidence. Never execute package code, install dependencies, run lifecycle scripts, import modules, run builds, invoke shells, or render package-provided active content. Workflow definitions read for release authority are repository content and get the same treatment: parsed by the bounded reader in `lib/release-authority/yaml.ts`, never evaluated.
