...
- `test/e2e/fake-registry.mjs` serves only the staged-publish, packument, and tarball endpoints the app uses. It also exposes a test-only control plane (`POST /__control/release/:stageId`) that "approves" a stage the way npm would — removing it from the stage listing and surfacing the staged version in the packument — so the release-reconciliation flow can be exercised end-to-end. Control requests are excluded from the request journal.
