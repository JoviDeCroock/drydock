...
- `test/e2e/build-fixtures.mjs` runs `npm pack --json` for the previous and staged package directories and writes generated registry state to `.context/e2e-registry/`. A scenario may rewrite the packed `package.json` (`staged.packageJsonText`), which repacks the tarball with the system `tar`. Fixture tarballs must hold only what npm would publish, so the builder packs with `COPYFILE_DISABLE=1` — macOS tar otherwise stores each file's extended attributes as an AppleDouble `._name` member, and a bare `._package` entry is itself a scanner finding — and fails the build if any tarball still carries packing-host entries.
