...
- `test/e2e/gate-setup.spec.ts` drives the Settings deep link and guided gate wizard against mocked GitHub responses. It covers repository, environment, ecosystem, workflow copy/manual fallback, and release-target handoff, plus the states the armed badge must not survive (a mapping removed from the GitHub App card, a failed re-check, an unreadable rule), a verification racing the ecosystem pin, admin bypass, and the `#gate-setup` deep link, all without contacting GitHub.
