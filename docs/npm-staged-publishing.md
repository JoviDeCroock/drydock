...
| `ungated`           | A release target the organization **still has configured** (npm or auto-detect, on an active GitHub App installation) has completed a gate review of this package, and no gate review of this version exists. The stage was produced outside the gated workflow.            |
| `unknown`           | The check could not run: the gate history read failed (`history-unavailable`), or npm's stage record was unavailable while the organization has a live npm-capable release target (`registry-record-unavailable`). Whether the stage went through the gate is not known.    |
...
organization that only uses the watchtower sees no change; one that deleted its
release target, uninstalled the GitHub App, or pinned the target to another
ecosystem stops seeing `ungated` for that package.

The record is advisory and additive: it never moves risk, findings, or a
decision, and a check that could not run is recorded as `unknown`
(`scan.gate_continuity.lookup_failed` when the history read failed) rather than
as no record, so a transient failure never reads as "not gated". A stage that
