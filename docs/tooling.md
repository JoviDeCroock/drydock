...
- `test/dev-server-route-parity.test.mjs` — the e2e dev server routes every
  prefix `SERVER_OWNED_PATH_PREFIXES` (`server/index.ts`) claims to the Worker, and
  production keeps `run_worker_first: true`. A prefix added to the Worker but not the
  harness is answered locally by Vite's SPA fallback with a `200` HTML document, so
  the route looks alive in a browser and its handler never runs.
