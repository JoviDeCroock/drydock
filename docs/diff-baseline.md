...
"Any complete AI review result" is literal. On the staged-publish path the review is deferred (see [`architecture.md`](./architecture.md#advisory-ai-review-is-off-the-critical-path)), so a freshly persisted scan carries `ai_json.status = "pending"` and both `releaseRisk` and `artifactRisk` are the deterministic grades — pending contributes nothing and is never read as a completed clean review. When the follow-up lands, the grades are recomputed and floored against what was already persisted, so the AI can raise them and never lower them.
