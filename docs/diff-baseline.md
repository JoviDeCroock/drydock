...
Compare writes are size-guarded the same way the anonymous `/diff` payload is, and for the same reason: KV values cap at 25 MiB, and an oversized `put` was rejected inside a swallowed `.catch()`, so the entry silently never existed and **every** `/compare` and `/compare/file` request re-downloaded and re-parsed the whole published tarball. `serializeCompareCachePayload` now sheds display samples cheapest-first until the payload fits, flagging the records that lost one with `sample-omitted` (the same flag the diff view already renders) and setting `textSamplesOmitted` on the payload. File metadata is the floor: a payload with no samples still beats no cache entry. If even that floor is over the cap the write is skipped with `compare_cache.write_skipped_oversize` rather than spent on a `put` KV will reject, and a rejected write now logs `compare_cache.write_failed` instead of vanishing. The budget arithmetic and the byte counters are shared with `/diff` (`server/lib/review/sample-budget.ts`, `server/lib/platform/json-size.ts`) rather than duplicated; `/diff` additionally prioritizes changed paths over unchanged ones, which the compare payload cannot do because it holds only one side of the diff.

Sample shedding is a display-cache optimization, not a change to release-delta semantics. The compare metadata route bypasses a shed entry and reacquires the complete baseline before computing finding annotations; file-body requests can continue using the bounded cached payload. Serialization also returns the reduced object it just built instead of parsing the up-to-20-MiB JSON string back into a second object graph in the Worker isolate.
