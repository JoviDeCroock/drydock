...
- **Scan detail** — a one-row verdict strip (recommendation, only action-relevant
  ...
  header does not repeat the nested section names. Quiet terminal npm states and
  their observation time stay in the header metadata instead of taking a
  duplicate row. The decision button lives in the strip on a completed review
  and in the page header otherwise, since a failed gate review renders no strip.
