...
6. Risk signals render below the diff and are annotated with the file's diff status plus a best-effort line number when the deterministic rule can locate the evidence in the staged text sample. The scan detail API returns both full-artifact risk and release-delta risk; the page leads with release-delta risk, shows signals on added or modified files first, and groups unchanged or otherwise contextual signals under "Package context" so deterministic full-artifact findings stay visible without dominating a harmless release diff.
...
- The recent reviews list paginates with `GET /api/v1/scans?cursor=…&filter=…`. The default filter is `undecided`; filter chips (Undecided / Approved / Blocked / All) sit above the table and re-fetch on change. Rows include compact evidence counts (`changed files · findings`). Retries appear as separate rows — the old "newest-per-stage" dedup is gone now that decisions and pagination both depend on a one-row-per-scan model.
