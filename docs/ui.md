...
3. A timeline section renders persisted `scan_events` in chronological order from sanitized event metadata, excluding view events. It uses compact timestamp + badge rows so users can see queued, started, npm access, retry, failed, completed, and decision milestones without turning the page into an audit-log table.
...
7. Persisted report sections (reviewer notes, fingerprint, manifest changes) follow as section-label blocks, not cards. The old "Changed files" table was retired — that information is now available in the tree.
...
- The recent reviews list paginates with `GET /api/v1/scans?cursor=…&filter=…`. The default filter is `undecided`; filter chips (Undecided / Approved / Blocked / All) sit above the table and re-fetch on change. Completed rows lead with release risk, show artifact risk only when it differs, and use compact evidence counts (`changed files · release / context`). Retries appear as separate rows — the old "newest-per-stage" dedup is gone now that decisions and pagination both depend on a one-row-per-scan model.
