...
- Workspace setup lives on `/dashboard/settings`. npm access appears before the GitHub App setup so a maintainer can add the release-evidence token first. The dashboard shows a compact npm-missing callout with an explicit “Open settings” affordance when the active organization has no npm connection.
