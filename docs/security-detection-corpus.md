...
- `pypi.*` findings come from `pyPiReleaseFindings` and carry `PYPI_RULES_VERSION` (currently `0.5.0`).
...
`file.secret-content` signals. Windows carry real source context so slice seams cannot manufacture
word boundaries or anchors, while a linear ordered-signal scan preserves arbitrarily padded shell
pipelines. Private-key findings require a complete armored block; the
`legit-pem-delimiter-constant` benign hard-negative keeps parser constants quiet. The
`shell-download-execute-dropper` and `obfuscated-dynamic-fetch` golden cases pin the affected rule
families; direct scan-cost and redaction regressions cover adversarial padding, cross-window packed
payloads, and large armored private keys. The same bounded matcher now covers PyPI `setup.py` and VS
Code adapter findings, versioned as `PYPI_RULES_VERSION` `0.5.0` and `VSCODE_RULES_VERSION` `0.2.0`.
