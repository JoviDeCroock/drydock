...
`1.29.0` adds bounded dependency-artifact review for newly introduced npm dependencies. The
...
`1.30.0` hardens dependency-artifact coverage: clipped dependency files fail visibly instead of being
...
`1.31.0` closes two more dependency-artifact completeness gaps. Files retained hash-only after the
...
`1.32.0` preserves the critical dependency integrity signal even when the same artifact is also
...
`1.35.0` closes the remaining same-name dependency-review bypasses and integrity false positive. An
...
`1.36.0` closes a dependency-artifact completeness gap and aligns the gate and report with the precise
...
`1.38.0` aligns dependency evidence with npm's install selection and closes two release-risk bypasses.
