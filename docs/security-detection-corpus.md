...
- shared `file.*` / `code.*` / `diff.*` findings come from `deterministicFindings` and carry
  `DETERMINISTIC_RULES_VERSION` (currently `1.80.0`).
...
`1.56.0` bounds manifest web-accessible-resource expansion, traverses browser
documents through direct dependency edges, follows unqualified `open()` calls
and executable XHTML or standalone SVG documents, and models HTML, SVG, and
MathML namespaces plus script MIME types. The browser navigation and HTML
namespace golden cases pin the new positive and hard-negative boundaries.

`1.57.0` applies XML self-closing rules to XHTML documents, follows executable
scripts across SVG `title`/`desc` and MathML-to-SVG integration points, and
keeps script-shaped CDATA content inert. The browser navigation and HTML
namespace golden cases pin the added positive and hard-negative boundaries.

`1.58.0` replaces the browser document parser with a deliberately conservative
flat token scan. Consumer edges only ever cancel the test-path demotion —
findings are demoted, never dropped — so the failure directions are asymmetric:
a missed edge lets a hostile package keep reduced severity, while an invented
edge merely reports an inert reference at full severity. Bit-exact emulation of
engine parsing is an unbounded goal, so the scanner no longer models
namespaces, foreign content, raw text, CDATA, templates, comments, script MIME
types, self-closing syntax, or navigation-suppressing attributes. Every
URL-valued `src`/`href`/`data`/`action`/`formaction` attribute (plus
`srcdoc` and refresh-shaped `content` values) on any tag-shaped token counts,
tag and attribute names match on their local name, and sources resolve against
the document URL plus every declared `<base>` candidate (bounded, failing
loudly when exceeded). **This precision boundary is deliberate: reports that
the scanner follows an edge no browser would take are by design; only a missed
edge is a bug.** The browser navigation and HTML namespace golden cases pin the
over-approximation.

`1.59.0` follows extensionless packaged documents reached through literal
browser navigation and supplements valid XHTML/SVG documents with a bounded,
non-executing XML parse. XML qualified names therefore use the complete parser
grammar, and internal general-entity replacements count both as URL candidates
and as possible replacement markup. Entity count, recursion depth, and expanded
...
`1.62.0` tokenizes multiline static ESM declarations and retains the active
execution base across classic-worker `importScripts()` chains, nested Worker
...
`1.64.0` follows literal packaged resources assigned with `setAttribute()` and
...
`1.66.0` follows static `new URL(..., import.meta.url)` values used by dynamic
imports and DOM navigation or executable-resource sinks, and recognizes
...
`1.68.0` follows literal documents written with `document.write()` or
`document.writeln()`, assigned through `innerHTML` or `outerHTML`, or inserted
through `insertAdjacentHTML()`. Nested `.shtml` and `.shtm` resources are
traversed as browser-rendered HTML documents. The
`browser-literal-html-document-reachability` golden case pins both paths so
consumer-executable test-tree payloads retain full capability severity.

`1.69.0` follows literal documents appended through `innerHTML +=`, static
`new URL(..., import.meta.url).href` values, literal computed resource keys in
WebExtension option objects, and Unicode-escaped WebExtension identifiers. The
`browser-static-syntax-reachability` golden case pins all four syntax aliases so
consumer-executable test-tree payloads retain full capability severity.

`1.70.0` decodes JavaScript escapes in no-substitution template resource paths,
accepts those templates as literal computed WebExtension option keys, and follows
static `new URL(..., import.meta["url"])` aliases. The
`browser-static-syntax-reachability` golden case pins all three bypass regressions
so consumer-executable test-tree payloads retain full capability severity.

`1.71.0` accepts no-substitution templates as literal computed member names and
follows resource assignments through the node returned by
`appendChild(document.createElement(...))`. The
`browser-static-syntax-reachability` golden case pins both execution paths so
consumer-executable test-tree payloads retain full capability severity.

`1.72.0` preserves the owning extension document's execution base when a
literal DOM-created script is loaded. The `browser-static-syntax-reachability`
golden case pins a document-relative Worker created by that injected script so
its consumer-executable test-tree payload retains full capability severity.

`1.73.0` follows literal `location.pathname` assignments and static
`new URL(..., import.meta.url).href` values passed to tab or window navigation
options. The `browser-static-syntax-reachability` golden case pins both paths so
consumer-executable test-tree payloads retain full capability severity.

`1.74.0` follows literal paint, animation, layout, and audio worklet modules,
dynamically assigned form submission targets, and document-relative resources
resolved through a dynamically assigned `<base>` URL. The
`browser-dynamic-execution-reachability` golden case pins the worklet, form, and
base-URL paths so consumer-executable test-tree payloads retain full capability
severity.

`1.75.0` preserves every candidate document base when a dynamically loaded
script creates further relative resources and follows static
`new URL(..., import.meta.url).pathname` projections. The
`browser-dynamic-execution-reachability` and
`browser-static-syntax-reachability` golden cases pin both nested execution
paths so consumer-executable test-tree payloads retain full capability severity.

`1.76.0` follows executable elements returned by `insertBefore()` and CSS
worklet modules reached through `window.CSS` or `globalThis.CSS`. The
`browser-static-syntax-reachability` and
`browser-dynamic-execution-reachability` golden cases pin both execution paths
so consumer-executable test-tree payloads retain full capability severity.

`1.80.0` follows static `new URL()` values resolved against `location.href`,
`document.URL`, or `document.baseURI`, plus literal dynamic imports whose first
argument is preceded by comments. The
`browser-current-document-module-reachability` golden case pins both paths so
consumer-executable test-tree payloads retain full capability severity.
