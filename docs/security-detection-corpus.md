...
| `file.prompt-injection`                   | deterministic | anchor               | no              |
| `file.review-manipulation`                | deterministic | anchor               | yes             |
...
- Prompt-injection detection is phrase-based: paraphrased, encoded, non-English, or unusually segmented injection text raises nothing deterministically and falls to the AI reviewer, whose prompt treats injection attempts as reportable high/critical findings. Ordinary soft line breaks are normalized, but a phrase spread across more than the scanner's 4 KiB overlap remains outside deterministic coverage. Explicitly framed, tightly quoted threat examples and narrow defensive instructions to ignore untrusted content stay quiet; unqualified directives in strings, comments, or Markdown code spans still flag. Drydock schema tokens in ordinary API source or documentation stay quiet unless an explicit AI/Drydock review audience is told to produce them.
...
`1.45.0` adds bounded dependency-artifact review for newly introduced npm dependencies. The
...
`1.46.0` hardens dependency-artifact coverage: clipped dependency files fail visibly instead of being
...
`1.47.0` closes two more dependency-artifact completeness gaps. Files retained hash-only after the
...
`1.48.0` preserves the critical dependency integrity signal even when the same artifact is also
...
`1.49.0` closes dependency-artifact completeness and attribution gaps. A missing or unreadable root
...
`1.50.0` closes dependency-selection and install-reachability gaps. Required peers moved to a different
...
`1.51.0` closes the remaining same-name dependency-review bypasses and integrity false positive. An
...
`1.52.0` closes a dependency-artifact completeness gap and aligns the gate and report with the precise
...
`1.53.0` closes registry-resolution and dependency-evidence trust gaps. Published versions and ranges
...
`1.54.0` aligns dependency evidence with npm's install selection and closes two release-risk bypasses.
...
`1.55.0` closes two executable-content gaps in dependency review. Newly introduced direct dependencies
...
`1.56.0` separates dependency coverage, install execution, and risk observations from gate policy.
...
`1.57.0` closes three durable dependency-evidence trust gaps. Registry projection preserves the presence
...
`1.58.0` closes install-reachability gaps in dependency evidence. Active explicit and implicit node-gyp
...
`1.59.0` keeps dependency evidence bound to the bytes consumers install. A declared bundled child whose
...
`1.60.0` closes two more install-reachability gaps for omitted dependency bodies. Statically named local
...
`1.61.0` keeps partial dependency evidence fail-closed without erasing stronger retained-byte signals.
...
`1.62.0` keeps dependency evidence attached to the code consumers actually receive across additional
...
`1.63.0` aligns static install reachability with the loader and process forms the completeness detector
...
`1.64.0` closes two remaining omitted-body bypasses in dependency install reachability. Static Node
...
`1.65.0` keeps computed dependency install targets fail-visible across syntax boundaries. Escaped
...
`1.66.0` closes the cross-syntax edges left in dependency install completeness. Shell interpreters
...
`1.67.0` closes dependency completeness gaps across JavaScript, lifecycle shell syntax, and npm range
...
`1.68.0` closes dependency selection and install completeness gaps at syntax and version boundaries.
...
`1.69.0` aligns added-dependency artifact review with the shared deterministic finding contract.
...
`1.70.0` makes dependency-artifact evidence declaration-scoped and report-bounded. Section, name,
...
`1.71.0` restores the full newly-installed declaration selector behind dependency-artifact review:
