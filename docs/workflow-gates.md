...
- runs shared JavaScript, secret, native-artifact, suspicious-archive, and file-diff rules, treating manifest-loaded background scripts, token-validated classic-worker `importScripts()` dependencies, content scripts, extension pages (including root-relative popup, options, and sandbox paths), packaged pages opened through extension-page links, refresh metadata, literal DOM navigation (including `runtime.getURL()` values and `self`/`globalThis` window aliases), or literal `tabs.create({ url })`, `tabs.update({ url })`, and `windows.create({ url })` calls (including literal `runtime.getURL()` and legacy `extension.getURL()` wrappers), pages nested through frames, HTML objects, or embeds, iframe `srcdoc` scripts resolved against their inherited document base, direct or `import.meta.url` Worker paths (including plain Worker paths from generated Manifest V2 background pages), and packaged script paths injected through direct or global-object-qualified dot/bracket notation in `tabs.executeScript()` or Manifest V3 scripting APIs as consumer entrypoints regardless of their directory names, while keeping inert or raw-text HTML descendants and trailing-solidus lookalikes out of that executable graph;
