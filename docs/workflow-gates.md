...
- runs shared JavaScript, secret, native-artifact, suspicious-archive, and file-diff rules, treating manifest-loaded background scripts, token-validated classic-worker `importScripts()` dependencies, content scripts, extension pages (including root-relative popup, options, sandbox, XHTML, standalone SVG, and suffixless packaged paths), packaged pages opened through HTML or SVG links, refresh metadata, literal `runtime.getURL()` dynamic imports, literal DOM navigation (including unqualified `open()`, `runtime.getURL()` values, semicolonless assignments, and `self`/`globalThis`/`top`/`parent` window aliases), or literal `tabs.create({ url })`, `tabs.update({ url })`, and `windows.create({ url })` calls (including literal `runtime.getURL()` and legacy `extension.getURL()` wrappers), pages nested through HTML frames, objects, or embeds, iframe `srcdoc` scripts resolved against their inherited document base, direct or `import.meta.url` Worker paths (including plain Worker paths from generated Manifest V2 background pages), and packaged script paths injected through direct or global-object-qualified dot/bracket notation in `tabs.executeScript()` or Manifest V3 scripting APIs as consumer entrypoints regardless of their directory names, with document references collected by a deliberately conservative flat token scan supplemented by bounded XML qualified-name, decoded internal-entity, and inherited `xml:base` handling for XHTML/SVG — script MIME types, namespaces, raw text, CDATA, templates, comments, and self-closing syntax are not modeled, so every URL-shaped reference in a reachable document counts and only a missed edge is a defect (see the `1.58.0` through `1.60.0` notes in [`security-detection-corpus.md`](./security-detection-corpus.md)); manifest-declared web-accessible resources are expanded within fixed declaration, XML-entity, and wildcard-work budgets so hostile manifests cannot cause unbounded review work;
