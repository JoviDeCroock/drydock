...
- runs shared JavaScript, secret, native-artifact, suspicious-archive, and file-diff rules, treating manifest-loaded background scripts, classic-worker `importScripts()` dependencies, content scripts, popups, options pages, developer-tools pages, sidebars, side panels, URL overrides, and packaged pages nested through iframes as consumer entrypoints regardless of their directory names;
