...
Registry absence and transient fetch failures remain pending and are retried by the 15-minute cron backstop for seven days after approval. Multi-file registries can expose a partially uploaded release, so the first observed disagreement starts a durable 15-minute propagation grace period (the approval timestamp is not used because GitHub callback delivery can delay the start of publishing). A disagreement that survives the grace period writes one durable `scan.registry_digest_mismatch` event and emits the `workflow_gate.registry_digest_mismatch` error alarm with package/version and the reviewed/published digest sets. It never sets `registry_verified_at`, and therefore never upgrades public identity.
