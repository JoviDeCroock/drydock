...
- runs shared JavaScript, secret, native-artifact, suspicious-archive, and file-diff rules, treating manifest-loaded background scripts, classic-worker `importScripts()` dependencies, content scripts, extension pages (including root-relative popup and options paths), packaged pages nested through iframes, and literal script paths injected through `tabs.executeScript()` or the Manifest V3 scripting API as consumer entrypoints regardless of their directory names;
