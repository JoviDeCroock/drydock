...
Marketing attribution (`marketing_referrals`) is a counter table, not an event log. The only public surfaces recorded are the landing page, the docs page, and the package diff; authenticated routes are never recorded. A request is reduced to `(UTC day, surface, channel bucket)` where the bucket comes from a closed allowlist — no IP, no full referrer URL, no user agent, no session identifier, and no per-visit row is stored, so the data cannot be joined back to a person. An unrecognized `utm_source` collapses to `other` rather than creating a new storage key.
