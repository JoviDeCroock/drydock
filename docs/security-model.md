...
`GET /api/public/v1/package-diff/verdict` shares every property above and adds one restriction of its own: it is the machine-readable projection (`drydock.verdict.v1`) that dependency-update tooling reads at scale, so its response carries only counts, risk tiers, the capability delta, coverage accounting, declared source binding, and publication timestamps — never finding evidence, reasons, or rule identifiers, and never a grade stronger than `needs-review`. Automating public statements about third-party releases is the threat this posture answers: rule prose stays on the `/diff` page a human chooses to open. A cached pair costs only the verdict's own per-IP bucket; a cold pair additionally spends the shared expensive computation budget, exactly like a cold `/file` request.

`GET /public/reviews/:ecosystem/:package/:version` is the D1-backed companion for consumer policy. It returns only the `drydock.review-lookup.v1` schema tag and a boolean, is covered by the public-report per-IP limiter, and requires the exact version to have an active share plus threat-feed listing. It accepts only registry-established package identity: a workflow-gate scan is manifest-claimed and can never satisfy the lookup, so an unrelated organization cannot list an artifact under someone else's package name to manufacture maintainer-review evidence.
