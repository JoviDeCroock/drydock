...
Four further anonymous route groups exist under `/public`, all serving only data an organization owner/admin explicitly opted into publishing:
...
- `GET /public/reviews/:ecosystem/*?version=&digest=` — an artifact-exact machine lookup used by consumer policy checks. Its D1 query starts from `public_package_key`, which exists only while the scan is feed-listed, rather than searching all scans and filtering private rows afterward, and requires the requested digest to match verified staged npm integrity (`sha1`) or one recomputed workflow-gate artifact (`sha256`). Version equality alone never authorizes a hit because a stage can be rewritten before publication and a gate release may contain several artifacts. Never scanned, private, unlisted, revoked, wrong-version, and wrong-bytes states therefore share one successful `{ listed: false }` result; this remains distinguishable from transport unavailability so an outage policy cannot override a listed-review requirement. A hit exposes the matched digest, package-identity strength, the normalized intent-envelope tier, timestamps, and the capability-bearing public report URL—no organization/user/internal scan identity—and is `no-store` so withdrawal is immediate.
...
The badge and threat feed are colo-cached for 300s and purged on revoke/unlist. That purge is **colo-local and best effort** — it clears the region that served the revoking request, and every other region serves the withdrawn body until the TTL expires (longer still behind a badge proxy's own cache). The listed-review lookup and report itself have no such window. Treat the badge and feed as eventually consistent; the lookup/report routes are the authority.
...
- Public report sharing is an explicit owner/admin opt-in per completed scan; the listed-review lookup, badge, and threat feed are a second opt-in on top of it. See `public-reports.md` and the authorization posture section above.
