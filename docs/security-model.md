...
The same policy, and the same absence of credentials, covers atpm's staged review and workflow gate: a release candidate is a public record in the publisher's repository read through the identity above, so those paths add reachable hosts but no new trust and no new secret. Build attestations read on any of these paths are verified against a _pinned_ Sigstore root using a bounded DER reader (`server/lib/platform/x509.ts`) that parses only the certificate fields a signature check needs; a bundle is attacker-supplied data, so every malformed input becomes a displayable "does not verify" verdict rather than an exception or a page failure. Verification work is capped at 64 bundles for each published record or staged listing, and a PDS listing page cannot exceed its requested 100 records. A workflow gate also binds the selected record CID and signed subject digest through artifact download, so a rewritten record or copied attestation fails closed before approval. Transparency-log inclusion is not verified — the log entry supplies only the signing timestamp used to evaluate the short-lived Fulcio leaf's validity window. A third anonymous surface sits alongside the two above: `/stage/atpm/<publisher>/<rkey>` and `GET /api/public/v1/package-diff/atpm-stage`, which resolve a staged atpm candidate to its review. They read only public records in the publisher's own AT Protocol repository, attach no credentials, persist nothing, and create no session; abuse control is the same per-IP rate limiting as the rest of `/diff`, and the review they redirect to is an ordinary public diff whose `to` version happens to name a staged record. Requiring a session there would gate a deterministic diff of public bytes behind an account, at the one moment it is worth reading — before the release is published.

Connecting an atpm publishing account runs an AT Protocol OAuth sign-in (PAR, PKCE, DPoP) against the authorization server that account's own PDS delegates to, and then discards the tokens: the flow is run for its `sub` claim, which proves control of the DID, and Drydock needs no delegated access because everything it reads afterwards is public and it never writes. The proof decides ownership — whose releases appear in which organization's dashboard — not access. The only stored secret is the ephemeral DPoP private key held for the minutes one authorization request is in flight, sealed under its own HKDF domain and deleted when the request is consumed. The live stage subscription is a discovery hint only: an event names a DID, and the review that follows resolves and re-fetches everything from the publisher's own PDS. See [`atpm-trusted-publishing.md`](./atpm-trusted-publishing.md).
