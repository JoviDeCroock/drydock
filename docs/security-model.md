...
The npm version-status endpoint (`docs/registry-version-status.md`) documents a publish-access requirement and returns authorization failures as `404`. That does not widen this stance: the lookup is advisory, fails closed to "no status", and a read-only token that cannot ask simply gets nothing. Do not broaden the requested scope to enable it. The trusted-publisher config lookup (`GET /-/package/{name}/trust`, what `npm trust list` calls) follows the same rule from the same Worker-side broker: token attached only to the connection's registry, one attempt, bounded body, and every failure persisted as `unavailable` (or `unsupported` on a custom registry) rather than as a finding. The previous version's provenance is read from npm's public attestation route with no credentials and no signature verification, and is labelled as such.
