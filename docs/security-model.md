...
- `GET /public/reports/:token`, `GET /public/reports/:token/file?path=`, and `GET /public/reports/:token/attestation` — capability URLs. The unguessable 256-bit share token _is_ the authorization; unknown, malformed, and revoked tokens are indistinguishable 404s. They serve the canonical report export and, one path at a time, the redacted staged file samples that export deliberately omits — never scan events or org/user identifiers. They attach no credentials, persist nothing, are `no-store` so revocation is immediate, and share one per-IP rate-limit bucket. `GET /public/attestation-key` returns public key material only.
- The `file` route is the one anonymous surface that discloses package bytes from a private review, so it is bounded by construction rather than by its own checks. It reads through the same `getScanFile` the authenticated `GET /api/v1/scans/:id/file` uses, which resolves the scan's persisted `files.json` artifact: redaction and the sandbox's per-file retention caps were both applied at persist time, so the public route cannot serve a larger or less-redacted sample than the maintainer read. It returns exactly one entry of that artifact's file set — a path outside it is the same 404 as a bad token, so the route is an oracle for neither the token space nor a package's file list — and it never fetches a baseline, because reaching a published previous version spends the organization's npm credentials and no public route holds one. Adding this widened sharing from "the evidence a finding quotes" to "the reviewed sample of any file in the release"; the share dialog states that before a link is created, and revoke is immediate on this route as on the report.
