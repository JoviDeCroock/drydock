...
The publication monitor's cron sweep (and its authenticated **Check npm** button) is a second credential-free egress path, reached by no anonymous request: it reads public npm metadata and, only for a release the organization has a Drydock record of, that release's tarball. It never attaches a credential, never reads an npm connection token, and has no redirects (`redirect: "manual"`; a 3xx is unavailable evidence). Metadata comes from the fixed public origin (the loopback fake registry only when the local-development override is enabled), and a tarball URL must pass the published-tarball origin policy before it is fetched. Responses are capped at 4 MiB (metadata) and 16 MiB (tarballs) with a 5-second deadline each, bytes are only hashed, and each tick is bounded to a per-organization round-robin share of 24 watch checks, at most three tarballs per check, and a one-minute start window. See [`publication-monitor.md`](./publication-monitor.md).
