...
There are two deliberate exceptions, and they are different in kind.

The `/api/ci/v1/*` endpoints (push-path CI ingest, see [`ci-action.md`](./ci-action.md)) are **authenticated but not by a session** — a workflow runner has no cookie and sends no `Origin` header, so they are mounted ahead of the session and CSRF middleware and authenticate with a GitHub Actions OIDC token instead. The token's signature, issuer, and audience are all verified before any claim is read, and the organization is derived from the signed `repository_id` claim by looking up an existing release target; a caller-supplied organization is never trusted. A repository with no release target is refused, and one mapped by two organizations fails closed rather than picking a tenant. Authorization keys on the numeric repository id, not the name, because GitHub frees a renamed repository's name but never reissues its id. A token also cannot upload into another run's release set: the set records the run and attempt it was opened for, and every subsequent call re-checks them. Abuse controls are per-organization D1 rate-limit buckets for opening and uploading, plus hard caps on artifact size, artifact count, and total release bytes. Uploaded bytes are recomputed-digest-checked at ingest, parsed only in the credentials-free sandbox, and deleted once the review completes — the digests persist as provenance, the package contents do not.
