...
The reviewer's risk is attributed through the files its findings cite. Only a finding on this release's delta reaches `releaseRisk`, the score the workflow gate reads; a review whose findings all cite unchanged files raises the artifact headline but cannot reject a gate that nothing in the release changed. That keeps a longstanding string in package context — an old injection attempt included — from re-rejecting every subsequent release through the advisory path when the deterministic layer has already scoped it as context.

The reviewer's contribution is also bounded by its own verdict: `nothing_unusual` adds nothing, `review_recommended` at most medium, `suspicious` at most high, and only `blocked` reaches critical, on the artifact, release, and context scores alike. Release attribution is file-level — a line an AI finding records never moves it out of the release, so a decoy call on unchanged code cannot demote a newly added one. None of this lowers a deterministic score: the bound applies to the AI's upgrade before the max with the deterministic side. The reviewer's `deterministicAssessments` (confirmed/disputed notes on individual rule findings) are display-only, shown only when they name a rule finding the scan reported, and never move a score.
...
A verified email address is not a condition of signing in. Blocking sign-in put a mail round-trip in front of a product an account cannot evaluate without signing in, so an unverified account can sign in, open the dashboard, and run published-pair reviews — which are credential-free and touch nothing but public registry data.

Verification is instead required per action, by `requireVerifiedEmail` (`server/lib/auth/email-verification.ts`), on the requests that either spend trust the address is the only proof of or reach a third party: connecting and validating an npm token, recording a scan decision, creating a public share link, inviting an organization member, and starting a GitHub App install. Those answer `403 { code: "email_verification_required" }`. Accepting an invitation already required it, and still does — checked against the user row rather than the session, since an invitee's account may predate the session in hand.
...
- `GET /public/reports/:token`, `GET /public/reports/:token/file?path=`, and `GET /public/reports/:token/attestation` — capability URLs. The unguessable 256-bit share token _is_ the authorization; unknown, malformed, and revoked tokens are indistinguishable 404s. They serve the canonical report export and, only when that share's persisted opt-in permits it, one redacted staged file sample at a time — never scan events or org/user identifiers. They attach no credentials, persist nothing, are `no-store` so revocation is immediate, and share one per-IP rate-limit bucket. `GET /public/attestation-key` returns public key material only.
- The `file` route is the one anonymous surface that discloses package bytes from a private review, so it is bounded by construction rather than by its own checks. `public_share_includes_files` defaults false, preserving every capability issued before file diffs existed; new shares set it true, and an owner/admin can deliberately upgrade an existing token by re-sharing. A false flag returns the same 404 as a bad token. For opted-in shares, the route reads through the same `getScanFile` the authenticated `GET /api/v1/scans/:id/file` uses, which resolves the scan's persisted `files.json` artifact: redaction and the sandbox's per-file retention caps were both applied at persist time, so the public route cannot serve a larger or less-redacted sample than the maintainer read. It returns exactly one entry of that artifact's file set — a path outside it is the same 404 as a bad token, so the route is an oracle for neither the token space nor a package's file list — and it never fetches a baseline, because reaching a published previous version spends the organization's npm credentials and no public route holds one. The share dialog states the widened disclosure before a link is created or upgraded, and revoke is immediate on this route as on the report.
- `GET /public/badge/:ecosystem/*` — a shields.io endpoint payload for a package's latest review. Reflects only scans whose org took the _second_ opt-in (feed listing) on top of sharing, so a private share link never makes a scan name-queryable. The response carries no package name, no org identity, and no share token; an unlisted-but-scanned package and a never-scanned package return byte-identical bodies, so there is no enumeration oracle. Separate per-IP rate-limit bucket (badge proxies multiplex unrelated packages through shared egress addresses), and a throttled response says `unavailable` rather than impersonating `not reviewed`.
  - **The badge index is keyed by ecosystem and package name alone, so entering it requires proof the reviewing organization can publish under that name.** Only the credential-backed staged sources have it: the registry accepted the org's npm token for that exact name. A workflow-gate scan only claims the name in a manifest, so it can answer the badge but never displaces a staged review and renders visibly `unverified`. A published-pair review proves nothing at all — it needs no credential and any account can run one against any public release — so it is excluded from the index outright, at both the write (`badgeLookupKey` assigns it no `public_package_key`) and the read (`listBadgeCandidateScans` excludes the source). Adding a scan source that can occupy the badge is a security decision; `scanPackageIdentity` fails closed so a new one does not inherit the credential-backed identity by default. See `public-reports.md` for the full identity ladder.
...
The publication monitor's cron sweep (and its authenticated **Check releases** button) is a second credential-free egress path, reached by no anonymous request: it reads public npm metadata and, only for a release the organization has a Drydock record of, that release's tarball. It never attaches a credential, never reads an npm connection token, and has no redirects (`redirect: "manual"`; a 3xx is unavailable evidence). Metadata comes from the fixed public origin (the loopback fake registry only when the local-development override is enabled), and a tarball URL must pass the published-tarball origin policy before it is fetched. Both responses stream into bounded state, so memory stays flat: the package document into only the fields the verdict reads (64 MiB cap, 15-second deadline; tracked containers are at most four deep, unread values are skipped by a bracket counter with no per-level state, and an overlong field degrades only itself), tarball bytes into two hashes (256 MiB cap). Bytes are only hashed, and each tick is bounded to a per-organization round-robin share of 24 watch checks, at most three tarballs sharing one 30-second deadline per check, no new check or download once a sweep has read 1 GiB, and a one-minute start window. What cannot be verified within those bounds is reported to the organization as a coverage gap rather than settled silently. See [`publication-monitor.md`](./publication-monitor.md).
