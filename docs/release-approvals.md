...
- `GET /api/v1/scans/:id` returns an `approvals` object: `required`,
  `approvedCount`, `blockedCount`, `verdict`, the `approvals` roster (member,
  decision, reason, timestamp), `viewerDecision`, and `eligibleApproverCount`.
  ...
  identities.
- `GET /api/v1/scans` returns `requiredApprovals` and a per-row `approvalCount`,
  so a partially-approved release renders as "1 of 2" in the queue instead of
  "undecided". It stays in the undecided filter, which is exactly where the
...
- Scans decided before this existed have a decision and no votes. The readers
  ...
  decision never renders as "0 of 1 approved". The same fallback covers a gate
  package auto-blocked by the artifact verifier, which has no human voter.
