...
**Report republication.** When the review reaches any terminal state, including the fail-safe `unavailable` state, the report has to change: `aiFindings` and `risk` move, and a completed review may also change the index-based `findingAnnotations`. The patch writes a **new, content-addressed** `report.{digest16}.json` + `manifest.{digest16}.json` instead of rewriting in place, then flips `report_digest`, `report_artifact_key`, `artifact_manifest_key`, `artifact_manifest_digest`, and `artifact_manifest_size` in the same statement that flips `ai_status`. Rewriting in place would leave a window where R2 holds bytes whose digest is not the one D1 recorded, and the read path (correctly) refuses that pair — a compacted artifact-backed scan would serve metadata only until D1 caught up. With new keys the old pair stays valid until the single D1 write swaps it, so a reader sees either the pending report or the terminal one. Two concurrent follow-ups cannot collide either, since different reviews hash to different keys. The pre-AI `report.json` is left in place (it is small, and it is reclaimed by the per-scan prefix sweep). If republication fails, the existing report stays readable and D1 remains authoritative for the terminal review status and risk.
...
deliveries that produce identical bytes produce identical keys. A missing scan
row also proves that the revision is orphaned, closing the race where a delete
sweep runs just before a stale follow-up finishes writing new objects.
