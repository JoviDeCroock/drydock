...
The route processes small idempotent batches and returns counts for `scanned`, `backfilled`, `alreadyBacked`, `digestMismatch`, `failed`, and `nextCursor`. A legacy row is marked artifact-backed only after the reconstructed canonical report digest equals the persisted `report_digest`; rows that cannot be reconstructed exactly stay D1-backed. That final metadata update also refuses a concurrent retention claim. If retention deleted or kept the row D1-only while backfill was writing, backfill sweeps its unowned R2 prefix; if another backfill already installed artifact metadata, the deterministic winning objects are left intact.
...
1. Atomically claims the still-unshared, non-pending-gate row in D1. Public-share creation checks the same lease, so either sharing wins and retention defers, or retention wins and no capability URL is minted while teardown is in flight. A failure before any artifact evidence is removed releases the claim. A failure after R2 succeeds keeps the claim as a sharing tombstone but expires it immediately so the next tick can finish; an interrupted invocation's claim expires after an hour so a later tick can recover it.
2. Sweeps the per-scan R2 prefix. **If the sweep fails before removing evidence, the D1 row is left completely untouched** apart from the temporary claim (counted as `deferred`), the claim is released, and the next tick retries. A partial sweep failure keeps the immediately reclaimable sharing tombstone instead.
3. Clears the artifact-metadata columns, so the row stops claiming objects that are now gone.
4. Atomically deletes `scan_findings` and `scan_files`, detaches or deletes the
   scan's audit events according to their own window, then deletes the
   organization-scoped, claim-token-matched `scans` row. A failure rolls the whole D1 batch back.

Step 3 sits between the last two destructive operations because the state to design against is a live row that still claims to be artifact-backed after its evidence is gone: an artifact-backed row has no `scan_files` / `scan_findings` either, and `SCAN_ARTIFACT_READS_DISABLED` is not a recovery path for those rows, so it renders as a completed scan with zero files and zero findings while its `risk` and `finding_count` still advertise them. Clearing before the sweep (the previous order) traded that for an equally bad one: a sweep failure — the likeliest operation to fail, since it is the only step that leaves D1 — stranded a live, expired row that reads just as emptily, with its manifest key and digest already gone, so its still-present objects could no longer be re-linked if retention were switched off before the retry. Sweeping first means that failure leaves the row intact and correct. The sweep does not need the columns it clears: the prefix is derived from the organization and scan ids.

The residual states that remain are transient and self-healing, because a deferred scan is still past the cutoff: a failed clear leaves a claimed row pointing at swept objects until the next tick re-sweeps (a no-op) and clears; a failed atomic D1 teardown leaves its child rows and audit links untouched under the immediately reclaimable claim. The claim continues blocking public-share creation in both post-R2 states. An artifact-backed row is still metadata-only after its R2 sweep and metadata clear, but a degraded D1-backed row keeps its only file/finding evidence if the parent delete fails. The next tick retries the complete batch.
