...
4. Atomically detaches or deletes the scan's audit events according to their
   own window, then deletes the
   organization-scoped, claim-token-matched `scans` row. A failure rolls the whole D1 batch back.

Step 3 sits between the last two destructive operations because the state to design against is a live row that still claims to be artifact-backed after its evidence is gone. Clearing before the sweep would make the likeliest external failure strand a live row with its manifest key and digest already gone, so its still-present objects could no longer be re-linked. Sweeping first leaves the row intact on that failure. The sweep does not need the columns it clears: the prefix is derived from the organization and scan ids.

The residual states are transient and self-healing: a failed clear leaves a claimed row pointing at swept objects until the next tick re-sweeps and clears; a failed atomic D1 teardown leaves its audit links and scan row intact under the immediately reclaimable claim. The claim continues blocking public-share creation in both post-R2 states, and the next tick retries the complete teardown.
