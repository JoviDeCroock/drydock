...
2. send the resulting evidence to Kimi for AI review (migrate the `AI_MODEL` constant to the effective K2.6 model before enabling paid AI review — see the pricing-inputs note above), with Qwen as a secondary route only after transient Kimi capacity/overload/rate-limit failures exhaust bounded retries.
