...
- **D1 storage growth**: ~500 KB–2 MB per scan retained during the no-data-loss R2 rollout because D1 text samples remain as fallback. After artifact backfill and shadow-read confidence, D1 text-sample compaction can move most large derived evidence pressure to R2.
