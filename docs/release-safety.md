...
| Dependency-artifact review (selection, resolution, install observations, coverage gaps)      | Focused Node tests plus `test/fixtures/security-corpus/cases-dependencies/*.json` fixtures, keeping the benign and native-build calibration cases honest, and the fake-registry journal assertion for credential-free fetches |
