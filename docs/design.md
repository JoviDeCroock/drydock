...
- **Hero:** `ContentArticleHero` — accent `Eyebrow`, Display 1 headline, `17px` lead, optional `MonoDetail`, primary/secondary `LinkButton` pair. It closes with `pb-2`, not `py-12`: the page's own `gap-12` supplies the section gap, and doubling them left a visible void under the CTA.
