...
The push path inverts the gate's trigger: the CI action uploads the release candidate to `/api/ci/v1/*` over a GitHub Actions OIDC token as the build produces it, and Drydock reviews immediately instead of waiting for a job to be blocked. Uploads for one workflow run converge on a **release set** keyed by `(organization, repository, run, attempt, releaseKey)`, so a matrix build lands as one review; sealing that set is what makes it reviewable, which is how a monorepo avoids being scanned half-uploaded. From there the two paths are literally the same code — `prepareBundlePackages` splits the parsed artifacts per package and `reviewReleasePackages` (`server/lib/scan/review-packages.ts`) runs one scan per package against its own baseline, for gates and pushed releases alike. Uploaded bytes are digest-recomputed at ingest, parsed only in the credentials-free sandbox, and deleted once review completes; the digests persist as provenance. A later `deployment_protection_rule` delivery for the same run binds its gate to the finished review and collects the decision — instantly when a maintainer already approved every package, otherwise leaving the deployment held — and a run with no pushed release falls back to the pull path unchanged. See [`ci-action.md`](./ci-action.md).
...
- **D1** — Better Auth tables, organizations, npm connections, scans, scan files/findings, workflow gates, CI release sets and their artifact digests, release targets, summaries, and audit/event metadata. D1 remains the operational source of truth.
- **R2** — canonical report JSON and redacted file/diff artifacts, plus pushed CI release bytes for the short window between upload and the end of review. D1 keeps compact metadata and historical fallback samples so list/detail pages remain cheap.
