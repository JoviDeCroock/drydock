...
The sandbox also digests each archive's raw wire bytes as they stream past (`digestArchiveStream`, SHA-1 — the digest npm publishes as `shasum`) and returns it as `DownloadResult.archiveSha1`. This binds a review to the artifact it reviewed: the npm staged-publish adapter compares it against the digest npm recorded for the stage (`evaluateStagedTarballIntegrity`), so a truncated or substituted download raises `stage.tarball-digest-mismatch` instead of being reported as a publisher deleting files. The digest is deliberately not an inline tap that finalizes on flush — the tar reader stops at the end-of-archive marker and cancels, so the wrapper owns the source reader and keeps draining it after a cancel. It reports a digest only when it observed the source's own EOF, and null (never a partial digest) when the stream was cancelled upstream, errored, or ran past the digest cap, so an unverifiable download degrades to "unverified" rather than accusing a publisher of tampering.
