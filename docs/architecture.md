...
- `POST /api/v1/staged-publishes/scan` — discover open staged publishes and create scans for newly found stage IDs. The worker's `scheduled` cron trigger runs every 15 minutes only for npm connections with `validation_status = "valid"` and `staged_publishes_monitor_enabled = true`;
...
- `PATCH /api/v1/npm-connection` — update connection settings such as unattended staged-publish monitoring opt-in;
