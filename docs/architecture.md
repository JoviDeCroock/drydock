...
Peak memory is what caps reviewable package size — a scan holds both package sides in a 128 MiB isolate — so the pipeline is ordered to keep at most one copy of each side alive. `analyzeRelease` acquires both sides, diffs them, runs deterministic findings over **raw** text, projects what later phases need into `ArtifactFacts` (package summary, file counts, baseline), and releases the raw file arrays. `ResolvedArtifacts` never leaves that call, so AI review, risk scoring, report assembly (`stableJson` over the whole report), and persistence run with only the redacted copies reachable — `persistResults` takes the facts, not the artifacts, so it cannot reach an unredacted body even by accident. The order is load-bearing in the other direction too: findings must run _before_ redaction, and only redacted evidence is persisted or returned.

Registry packument reads on this path are cached for 5 minutes in `COMPARE_CACHE` (`fetchPackageMetadataCached`, shared with the compare/versions routes). Cache keys are organization-scoped (`org:<id>`), because a packument fetched with one organization's npm token may include private packages another must not see, and the scan path asks for npm's abbreviated packument (keyed separately, since only the full document carries per-version `time`). Every registry document is projected down to the fields Drydock reads (`versions[].dist.tarball`, `dist-tags`, `time`) at the fetch boundary, so a package with thousands of versions does not keep megabytes of readmes and per-version manifests resident for the length of a scan. A stale packument can only select an older baseline, which the report records in `BaselineInfo` and which makes the diff louder, never quieter.
