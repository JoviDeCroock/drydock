...
npm removes a stage from the staged-list endpoint when it is approved _or_ rejected, exposes no status field, and sends no webhook — and the npm-side approval always requires an OTP, so it cannot be automated. What Drydock can automate is the second half of the double-approval: detecting that a reviewed stage was released (or withdrawn) and resolving the scan without a manual Drydock decision. Every discovery sweep (cron and manual route alike) ends with `reconcileDisappearedStages` (`server/lib/staged-publishes-discovery.ts`): for each completed npm scan with no recorded release outcome whose `stage_id` is absent from the live stage listing, it fetches the packument and classifies the disappearance via `server/lib/release-detection.ts`:

- staged version present with a `dist.shasum` matching the persisted `scans.staged_shasum` → `release_status = "released"` (the shasum match doubles as an integrity check that the published bytes are the reviewed bytes);
- staged version present with a _different_ shasum → `release_status = "released_mismatch"` — something other than the reviewed artifact shipped under that version, so the scan stays in the review queue as an alert and is never silently resolved;
- staged version absent → the scan's `stage_missing_since` clock starts; if the version still hasn't surfaced after `STAGE_WITHDRAWN_CONFIRMATION_MS` (45 minutes, three cron cycles — covering registry propagation lag), the scan resolves as `release_status = "withdrawn"`. A stage that reappears in the listing clears the clock.

The outcome lives in `scans.release_status` / `scans.released_at`, deliberately separate from `scans.decision`: a registry observation must never forge a human verdict, and decided scans are still reconciled so a release after a `no_publish` decision is recorded rather than missed. Each resolution writes an audit event with no actor (`scan.release_detected`, `scan.release_mismatch_detected`, `scan.stage_withdrawn`). The dashboard's "undecided" filter drops `released`/`withdrawn` scans from the review queue (keeping `released_mismatch` visible), and the list and detail views badge the outcome. Transient packument fetch failures skip the scan until the next sweep; a 404 packument counts as "version absent". Workflow-gate scans are excluded — their lifecycle is the GitHub gate decision, not a stage in the registry.
