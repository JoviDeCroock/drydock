...
- **Detonation container** (optional, default-off) — a _separate_ Cloudflare Container that deliberately executes a package's install lifecycle for dynamic analysis. It is not the Worker and not the parsing sandbox: the Worker never runs package code. It receives package bytes only (never credentials or org identifiers), its report is re-validated as untrusted input, and its findings are advisory. See [`detonation.md`](./detonation.md).
