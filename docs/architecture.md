...
### Workers AI (gated-target surface, Flagship-gated)

Workers AI review is wired into the scan pipeline through `maybeRunAiReview`, but it only runs on the **gated-target surface** and is additionally **gated by the per-organization Cloudflare Flagship `ai-review` flag, off by default**. The reviewer module - `server/lib/ai-review.ts`, its model policy, the prompt-injection-resistant system prompt, the AI SDK evidence-tool loop, and the test suite - stays in the active contract for the planned paid-tier feature. The code currently declares `AI_MODEL = "@cf/moonshotai/kimi-k2.5"`; Cloudflare scheduled that model to alias to Kimi K2.6 on May 30, 2026, so treat K2.6 as the effective model and pricing target unless the constant is migrated.

`runScanPipeline` takes a `surface: ScanSurface` (`"staged-publish" | "gated-target"`, defined in `server/lib/scan-pipeline.ts`) that decides whether AI review may run at all:

- **`staged-publish`** (the `executeScanJob` hot path users drive with their own npm CLI/UI) is **deterministic-only**. `maybeRunAiReview` short-circuits before consulting Flagship — no AI cost, no added latency — and records the review as `unavailable` with summary "AI review does not run on the staged-publish path." Regex/constant-folding can't see runtime-assembled identifiers, but that semantic layer is deliberately _not_ paid for on the publish hot path.
- **`gated-target`** (the `executeWorkflowGateJob` GitHub deployment-protection gate — PyPI today, npm planned) is a gate, not the publish, so it may run the additive AI/AST semantic reviewer that catches the assembled-identifier class. Here, and only here, the `ai-review` Flagship flag is consulted per-organization.

This split is a product decision: see the staged-vs-gated detection direction and `docs/detection-eval.md`. When the gated-target surface is selected but Flagship does not return `true` for the scanning organization, the pipeline records an `unavailable` AI review with summary "AI review is disabled." If enabled AI review fails, the pipeline emits `scan.ai_review.failed`, records AI review as unavailable, and still persists the deterministic report. Risk is computed from deterministic findings unless a complete, schema-valid AI review returns additional evidence or an explicit manual-review flag. The AI reviewer is always additive — it can raise risk or add context but can never approve a release or downgrade deterministic findings.
