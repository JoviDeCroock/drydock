...
| `public-diff/`    | Anonymous `/diff` orchestration, its per-ecosystem acquisition, download/error/page helpers.                                                          |
| `adapters/`       | Per-ecosystem registry and artifact behavior (npm, PyPI, VS Code).                                                                                    |
| `workflow-gates/` | Shared GitHub Environment gate plumbing and per-ecosystem gate adapters.                                                                              |
...
Files that sit at `server/lib/` root are the ones no single group owns: the sandbox boundary (`sandbox.ts`, `tar-parser.js`), the npm registry surface (`registry.ts`, `published-tarball.ts`, `npm-connection.ts`, `staged-publishes*.ts`), the cross-cutting `compare-cache.ts`, and `workflow-gate-job.ts`.
