...
`/diff` is an anonymous marketing/utility surface: it diffs any two published versions of an npm package and runs the deterministic rule set over the newer artifact, reusing the scan pipeline's pure phases (`server/lib/public-diff.ts`, routes in `server/routes/public-diff.ts`). The endpoints are mounted before the auth middleware, use no credentials, and are enabled only when `NPM_REGISTRY` is the canonical public npm registry; deployments configured for a custom registry return 404 for this API. They persist no review data to D1 and never run AI review. Abuse control is per-IP rate limiting (expiring D1 `rate_limits` buckets), the shared anonymous tarball-byte cache described in [`diff-baseline.md`](./diff-baseline.md), and a versioned computed-result cache: an awaited `caches.default` write makes the first file navigation immediately available in the serving colo, while a 30-day `COMPARE_CACHE` KV entry supports cross-colo reuse. File-content cache misses share the stricter version-pair computation budget, while cache hits retain a higher navigation allowance. Cache keys include the deterministic-rules and risk-schema versions so detection changes cannot reuse stale assessments; oversized KV values omit file samples based on their UTF-8 byte size. Sandbox archive caps bound per-request work. Version-pair responses require origin revalidation so a deployment cannot leave browsers on an older analysis, while `Cache-Tag: public-diff:<package>` remains available for package-level purges.

The main version-pair JSON response has an off-by-default Workers Cache pilot behind `PUBLIC_DIFF_WORKERS_CACHE_PILOT=1`. The uncached default entrypoint still enforces the canonical-registry check, input validation, and per-IP D1 rate limit, then delegates a credential-free canonical request to the cached `PublicDiffReads` entrypoint. Only successful pair responses receive an edge-only one-day freshness window; browsers still revalidate, and errors are explicitly `private, no-store`. Wrangler enables caching only on that named entrypoint and deliberately has no top-level `cache` block, so the default Worker, static assets, `/versions`, and `/file` do not consult Workers Cache or acquire its additional request billing. Worker versions remain part of the cache key because cross-version caching is not enabled.

The static `/diff` shell is used for direct detail URLs, with Worker-side HTML rewriting of package-specific title, description, Open Graph, Twitter, and canonical metadata before the client renders. Findings run on raw text samples before redaction — the same order as the scan pipeline — and only redacted evidence is cached or returned.
