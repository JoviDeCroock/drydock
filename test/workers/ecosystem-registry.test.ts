...
describe("ecosystem capability registry", () => {
  ...
  test("bounds atpm computed pairs to its mutable resolution lifetime", () => {
    ...
    expect(adapter?.payloadVersion).toBe("v9");
    expect(adapter?.rulesVersionSegment).toContain("atpm-13+identity-3");
    ...
    expect(adapter?.rulesVersionSegment).toContain("provenance-6+publisher-1");
    expect(ATPM_RECORD_CACHE_SCOPE).toBe("atpm-public-record-13-provenance-6-absolute-expiry-v1");
