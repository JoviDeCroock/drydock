...
4. **Bump `DETERMINISTIC_RULES_VERSION`** in `server/lib/review/rules/index.ts` whenever rule semantics, severities, or coverage change. PyPI-only `pypi.*` rules instead bump `PYPI_RULES_VERSION` in `server/lib/ecosystems/pypi/index.ts`.
...
6. **Add the corpus fixture** at `test/fixtures/security-corpus/cases/<id>.json` (PyPI: `cases-pypi/`). Required fields: `id`, `title`, `category`, `intent`, `stagedFiles` (sandbox-shaped `FileRecord[]`), `expectedRisk` (from `computeRisk()`), and `expectedFindings` as exact `{ ruleId, severity, file }` tuples. Optional: `previousFiles`, `previousPackageJson`/`stagedPackageJson`, `expectedPackageJsonDiff`, `coverageGaps`. Safety policy (see `docs/security-detection-corpus.md`): synthetic `FileRecord` JSON only, `example.invalid` URLs, obviously fake secrets, never runnable malware.
